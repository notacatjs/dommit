...
function Reactive(el, model, opt) {
  if (!(this instanceof Reactive)) {
    return new Reactive(el, model, opt);
  }
  ...
  if (el instanceof window.DocumentFragment) {
    throw new Error('Only one top level element is allowed in template');
  }
 ...
 * @api public
 */
Reactive.prototype.appendTo = function(element) {
  var self = this;
